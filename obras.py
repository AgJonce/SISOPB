...
def incluir_empenho():
    ...
    st.title("➕ Incluir Empenho")

    st.caption(
        "Cadastre um novo empenho vinculado a uma obra."
    )
    ...
    for registro in obras:
        ...
        obra_id_registro = registro[0]
        nome_obra = registro[1] or ""
        contrato_obra = registro[2] or ""
        recurso_obra = registro[3] or ""
        valor_obra = float(
            registro[4] or 0
        )
        situacao_obra = registro[5] or ""

        descricao = (
            f"{obra_id_registro} - {nome_obra}"
        )
        ...
        dados_obras[
            ...
            "recurso": recurso_obra,
            "valor": valor_obra,
            "situacao": situacao_obra
        }
    ...
    with col1:
        ...
        st.text_input(
            ...
            disabled=True,
            key=f"contrato_empenho_{obra_id}"
        )
    ...
    with col2:
        ...
        st.text_input(
            ...
            disabled=True,
            key=f"recurso_obra_empenho_{obra_id}"
        )
    ...
    with col3:
        ...
        st.metric(
            "💵 Valor da Obra",
            f"R$ {dados_obra['valor']:,.2f}"
        )

    # =========================================================
    # EMPENHOS JÁ CADASTRADOS DA OBRA
    # =========================================================
    ...
    with col1:
        ...
        st.metric(
            "💼 Já Empenhado",
            f"R$ {total_empenhado_liquido:,.2f}"
        )
    ...
    if saldo_obra <= 0:
        ...
        st.warning(
            "⚠️ Esta obra não possui saldo disponível "
            "para novo empenho."
        )
    ...
    with col1:
        ...
        st.text_input(
            "🔢 Número do Empenho",
            value=numero_empenho,
            disabled=True,
            key=(
                f"numero_empenho_auto_"
                f"{obra_id}_"
                f"{ano_empenho}"
            )
        )
        ...
        tipo_empenho = st.selectbox(
            ...
            key=f"tipo_empenho_{obra_id}"
        )

        numero_processo = st.text_input(
            "📂 Número do Processo",
            key=f"processo_empenho_{obra_id}"
        )

        numero_licitacao = st.text_input(
            "⚖️ Número da Licitação",
            key=f"licitacao_empenho_{obra_id}"
        )
    ...
    with col2:
        ...
        credor = st.text_input(
            "🏢 Credor",
            key=f"credor_empenho_{obra_id}"
        )

        cpf_cnpj = st.text_input(
            "🪪 CPF / CNPJ",
            key=f"cpf_cnpj_empenho_{obra_id}"
        )

        banco = st.text_input(
            "🏦 Banco",
            key=f"banco_empenho_{obra_id}"
        )

        agencia = st.text_input(
            "🏦 Agência",
            key=f"agencia_empenho_{obra_id}"
        )

        conta = st.text_input(
            "💳 Conta",
            key=f"conta_empenho_{obra_id}"
        )
    ...
    with col1:
        ...
        unidade_orcamentaria = st.text_input(
            ...
            key=f"unidade_empenho_{obra_id}"
        )

        funcao = st.text_input(
            "📌 Função",
            key=f"funcao_empenho_{obra_id}"
        )

        subfuncao = st.text_input(
            "📌 Subfunção",
            key=f"subfuncao_empenho_{obra_id}"
        )
    ...
    with col2:
        ...
        programa = st.text_input(
            "📋 Programa",
            key=f"programa_empenho_{obra_id}"
        )

        acao = st.text_input(
            "🎯 Ação",
            key=f"acao_empenho_{obra_id}"
        )

        elemento_despesa = st.text_input(
            "💵 Elemento da Despesa",
            key=f"elemento_empenho_{obra_id}"
        )
    ...
    with col3:
        ...
        fonte_recurso = st.text_input(
            ...
            key=f"fonte_empenho_{obra_id}"
        )

        ficha_dotacao = st.text_input(
            "📑 Ficha / Dotação",
            key=f"ficha_empenho_{obra_id}"
        )
    ...
    with col1:
        ...
        valor_empenhado = st.number_input(
            ...
            step=0.01,
            format="%.2f",
            key=f"valor_empenhado_{obra_id}"
        )
    ...
    if valor_liquido_novo > saldo_obra:

        st.error(
            "🚨 O valor líquido deste empenho "
            "ultrapassa o saldo disponível da obra."
        )
    ...
    historico = st.text_area(
        "📋 Histórico",
        key=f"historico_empenho_{obra_id}"
    )

    observacao = st.text_area(
        "📝 Observação",
        key=f"observacao_empenho_{obra_id}"
    )
    ...
    if st.button(
        ...
        # =====================================================
        # RECALCULAR SALDO NO MOMENTO DO SALVAMENTO
        # =====================================================
        ...
        if (
            ...
            st.error(
                "🚨 O valor do empenho ultrapassa "
                "o saldo atual disponível da obra."
            )
            return
        ...
        try:
            ...
            # =================================================
            # GERAR NOVAMENTE O NÚMERO
            #
            # Isso é importante porque o número exibido na
            # tela pode ter ficado desatualizado antes do clique
            # em Salvar.
            # =================================================
            ...
            numeros_banco = cursor.fetchall()

            numeros_utilizados = set()

            for registro in numeros_banco:
            ...
            if duplicado:
                ...
                st.error(
                    "❌ O número automático do empenho "
